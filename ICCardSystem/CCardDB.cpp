...
bool CCardDB::Insert(const CCard & card)
{
	...
	strSql.Format(_T("INSERT INTO Card VALUES (%d,%d,%d,%d,'%s',%d,%d,%d,%d,datetime('now','localtime'),NULL,NULL)"), card.GetID(),card.GetUserID(),card.GetNo(),card.GetType(),card.GetDeadline(),card.GetDeposit(), card.GetTotalAmount(),card.GetState(),card.GetEmployeeIDOpenCard());
...
bool CCardDB::Select(const CString & strSql, std::vector<CCard> & vec_card)
{
	...
	if (nRow)
	{
		...
		if (nColumn == 12)
		{
			...
			{
				CCard card(_ttoi(vec2_strData[i][0]), _ttoi(vec2_strData[i][1]), _ttoi(vec2_strData[i][2]), _ttoi(vec2_strData[i][3]), vec2_strData[i][4], _ttoi(vec2_strData[i][5]), _ttoi(vec2_strData[i][6]), _ttoi(vec2_strData[i][7]), _ttoi(vec2_strData[i][8]), vec2_strData[i][9], _ttoi(vec2_strData[i][10]), vec2_strData[i][11]);
