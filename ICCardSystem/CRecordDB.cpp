...
bool CRecordDB::Insert(const CRecord & record)
{
	...
	strSql.Format(_T("INSERT INTO %s VALUES (%d,%d,'%s',%d,%d,%d,%d,'%s',%d,datetime('now','localtime'))"), m_strTableName, record.GetID(), record.GetUserID(),record.GetCardNo(), record.GetAmountOld(), record.GetAmount(), record.GetAmountNew(),record.GetDeposit(),record.GetNoteInfo(), record.GetEmployeeID());
...
bool CRecordDB::Select(const CString & strSql, std::vector<CRecord> & vec_record)
{
	...
	if (nRow)
	{
		...
		if (nColumn == 10)
		{
			...
			{
				CRecord record(_ttoi(vec2_strData[i][0]), _ttoi(vec2_strData[i][1]), vec2_strData[i][2], _ttoi(vec2_strData[i][3]), _ttoi(vec2_strData[i][4]), _ttoi(vec2_strData[i][5]), _ttoi(vec2_strData[i][6]), vec2_strData[i][7], _ttoi(vec2_strData[i][8]), vec2_strData[i][9]);
